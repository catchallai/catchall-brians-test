  ...
  try {
    ...
    (deckData.slides || []).forEach((slide, index) => {
      ...
      if (slide.type === 'cover') {
        ...
        const bulletPoints = slide.content.points.filter(p => p).map(point => ({
          text: point,
          options: { bullet: true }
        }));
