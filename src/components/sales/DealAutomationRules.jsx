...
export default function DealAutomationRules({ businessId }) {
  ...
  const createRuleMutation = useMutation({
    mutationFn: (data) => base44.entities.AutomationRule.create({
      ...data,
      business_id: businessId,
      entity_type: 'deal',
    }),
  ...
  const getTriggerLabel = (triggerId) => TRIGGERS.find(t => t.id === triggerId)?.label || triggerId;
  ...
  return (
    <Card className="glass-card">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button 
          size="sm" 
          onClick={() => { setShowModal(true); setEditingRule(null); setFormData({
            name: '',
            trigger_type: '',
            trigger_value: '',
            action_type: '',
      ...
      <CardContent>
        {rules.length === 0 ? (
          <p className="text-sm text-gray-500 text-center py-8">No automation rules yet. Create one to get started.</p>
        ) : (
              ...
              <div key={rule.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                    When {getTriggerLabel(rule.trigger_type)} {rule.trigger_value} → {getActionLabel(rule.action_type)} {rule.action_value}
                  </p>
                </div>
          ...
          <div className="space-y-4">
            ...
            {formData.action_type === 'move_stage' && (
              <div>
                ...
                <Select value={formData.action_value} onValueChange={(v) => setFormData({ ...formData, action_value: v })}>
                  <SelectTrigger className="mt-1">
            ...
            {(formData.action_type === 'increase_probability' || formData.action_type === 'decrease_probability') && (
              <div>
