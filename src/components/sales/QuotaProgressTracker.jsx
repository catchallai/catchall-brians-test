...
export default function QuotaProgressTracker({ quota, forecast }) {
  ...
  const getRiskLevel = () => {
    if (achievement >= 100) return { level: 'On Track', color: 'bg-green-100 text-green-700', icon: '✓' };
    if (achievement >= 75) return { level: 'Good', color: 'bg-blue-100 text-blue-700', icon: '→' };
    if (achievement >= 50) return { level: 'At Risk', color: 'bg-amber-100 text-amber-700', icon: '⚠' };
    return { level: 'Critical', color: 'bg-red-100 text-red-700', icon: '!' };
  };

  const risk = getRiskLevel();
  const confidenceColor = (forecast?.confidence_score || 50) >= 75 ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700';
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Revenue Target</span>
              </div>
              <span className="text-sm font-bold text-gray-900 dark:text-white">
                ${(quota.current_revenue / 1000).toFixed(0)}k / ${(quota.revenue_target / 1000).toFixed(0)}k
              </span>
            </div>
            ...
            <div className="p-3 bg-gradient-to-r from-violet-50 to-blue-50 dark:from-violet-900/20 dark:to-blue-900/20 rounded-lg">
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Projected Revenue</span>
                </div>
              ...
              {willMeetQuota ? (
                <p className="text-xs text-green-600 dark:text-green-400 mt-1">✓ On pace to meet quota</p>
              ) : (
