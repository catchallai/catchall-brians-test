...
export default function ContractorManagement() {
  ...
  const addMutation = useMutation({
    mutationFn: (data) => base44.entities.Contractor.create({
      ...data,
  ...
  const handleSubmitSchedule = () => {
    if (!selectedContractor || !scheduleData.project_name || !scheduleData.start_date || !scheduleData.end_date) return;
            ...
            {contractors.map(contractor => (
              <div key={contractor.id} className="p-4 rounded-lg border hover:bg-gray-50 dark:hover:bg-gray-800">
                ...
                <div className="flex gap-2">
                  <Button size="sm" onClick={() => handleScheduleContractor(contractor)} className="flex-1 gap-1">
                    <Calendar className="w-3 h-3" />
                    Schedule
                  </Button>
                  {contractor.portfolio_url && (
                    <Button size="sm" variant="outline" asChild>
                      <a href={contractor.portfolio_url} target="_blank" rel="noopener noreferrer">
            ...
            {upcomingSchedules.map(schedule => (
              <div key={schedule.id} className="p-3 rounded-lg border">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <h4 className="font-semibold text-gray-900 dark:text-white">{schedule.contractor_name}</h4>
                    <p className="text-sm text-gray-600 dark:text-gray-400">{schedule.project_name}</p>
                  </div>
                  <Badge>{schedule.status}</Badge>
                </div>
                <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                  <div>
                    {format(new Date(schedule.start_date), 'MMM d, yyyy')} - {format(new Date(schedule.end_date), 'MMM d, yyyy')}
                  </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setScheduleData({ ...scheduleData, project_name: e.target.value })}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setScheduleData({ ...scheduleData, estimated_hours: e.target.value })}
                placeholder="8"
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => setScheduleData({ ...scheduleData, deliverables: e.target.value })}
            ...
            <div className="flex gap-3 justify-end">
              ...
              <Button 
                ...
                disabled={!scheduleData.project_name || !scheduleData.start_date || !scheduleData.end_date}
              >
                Schedule
              </Button>
            </div>
