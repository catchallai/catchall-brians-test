...
import { 
  Users, Building2, Target, Calendar, Mail, Zap, Search, Link2, 
  FileSearch, Globe, Share2, Radio, MapPin, Newspaper, FileText, 
  BarChart3, PenTool, TrendingUp, FlaskConical, UserPlus, RefreshCw,
  Bell, Loader2, Sparkles, Rocket, Crown
} from "lucide-react";
...
const FEATURES = {
  ...
  trafficAnalytics: { name: 'Traffic Analytics', icon: BarChart3, tier: 'enterprise', category: 'SEO', default: false },
  ...
  const applyPresetMutation = useMutation({
    mutationFn: async (tier) => {
      const updates = Object.entries(FEATURES).map(([key, feature]) => {
        const shouldEnable = tier === 'enterprise' ? true :
                            tier === 'growth' ? (feature.tier === 'starter' || feature.tier === 'growth') :
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Feature Management</h2>
                      ...
                      return (
                        <div 
                          ...
                          className={`flex items-center justify-between p-3 rounded-lg transition-colors ${
                            enabled ? 'bg-emerald-50 dark:bg-emerald-900/20' : 'bg-gray-50 dark:bg-gray-800'
                          }`}
                        >
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                              enabled ? 'bg-emerald-100 dark:bg-emerald-900/40' : 'bg-gray-100 dark:bg-gray-700'
                            }`}>
                              <Icon className={`w-5 h-5 ${enabled ? 'text-emerald-600' : 'text-gray-400'}`} />
                            </div>
                            <div>
                              <p className={`font-medium ${enabled ? 'text-gray-900 dark:text-white' : 'text-gray-500'}`}>
                                {feature.name}
                              </p>
