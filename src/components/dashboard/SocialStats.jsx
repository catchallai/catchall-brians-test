...
export default function SocialStats({ mentions = [], accounts = [], posts = [] }) {
  ...
  const totalEngagement = mentions.reduce((sum, m) => sum + (m.likes || 0) + (m.comments || 0) + (m.shares || 0), 0);
  const totalReach = mentions.reduce((sum, m) => sum + (m.reach || 0), 0);
  const scheduledPosts = posts.filter(p => p.status === 'scheduled' || p.status === 'draft').length;
  ...
  const stats = [
    { label: 'Mentions', value: formatNumber(totalMentions), icon: MessageSquare, color: 'text-blue-600' },
    { label: 'Engagement', value: formatNumber(totalEngagement), icon: Heart, color: 'text-pink-600' },
    ...
    <Card className="border-0 shadow-sm">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge className={`${
            sentimentScore >= 60 ? 'bg-emerald-100 text-emerald-700' :
            sentimentScore >= 40 ? 'bg-amber-100 text-amber-700' :
