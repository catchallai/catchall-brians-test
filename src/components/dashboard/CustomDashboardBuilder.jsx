...
import { 
  Plus, GripVertical, X, Save, Layout, 
  BarChart3, TrendingUp, Users, Target, Link2, Share2, Mail, PieChart
} from 'lucide-react';
import {
  Dialog,
  DialogContent,
...
const AVAILABLE_WIDGETS = [
  ...
  { id: 'competitor_comparison', name: 'Competitor Comparison', icon: Users, category: 'competitive' },
...
export default function CustomDashboardBuilder({ 
  ...
  const [selectedWidgets, setSelectedWidgets] = useState(
    dashboard?.widgets?.map(w => w.type) || ['seo_score', 'keyword_rankings', 'traffic_chart', 'deal_pipeline']
  );
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-6">
          {/* Name */}
          <div>
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Dashboard Name</label>
            <Input 
              value={name}
          ...
          {/* Layout */}
          <div>
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 block">Layout</label>
          ...
          <div>
            ...
            <div className={`grid gap-2 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg ${
              layout === 'grid' ? 'grid-cols-3' : layout === 'list' ? 'grid-cols-1' : 'grid-cols-4'
            }`}>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button onClick={handleSave} disabled={isLoading || !name || selectedWidgets.length === 0}>
