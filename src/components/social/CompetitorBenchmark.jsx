...
export default function CompetitorBenchmark({ 
  ...
  const getComparisonData = () => {
    ...
    return competitors
      ...
      .map(c => {
        const totalFollowers = c.social_accounts?.reduce((sum, a) => sum + (a.followers || 0), 0) || 0;
        const avgEngagement = c.social_accounts?.length > 0
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
            {competitors.map(comp => {
              const totalFollowers = comp.social_accounts?.reduce((sum, a) => sum + (a.followers || 0), 0) || 0;
          ...
          {selectedCompetitors.length < 2 && (
            <p className="text-xs text-amber-600 text-center mt-2">Select at least 2 competitors to compare</p>
          )}
        </CardContent>
      </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={200}>
                  <RadarChart data={[
                    { metric: 'Content', ...comparisonData.reduce((acc, c) => ({ ...acc, [c.name]: c.content_score }), {}) },
                    { metric: 'Brand', ...comparisonData.reduce((acc, c) => ({ ...acc, [c.name]: c.brand_strength }), {}) },
                    { metric: 'Engagement', ...comparisonData.reduce((acc, c) => ({ ...acc, [c.name]: c.engagement * 10 }), {}) }
                  ]}>
                    <PolarGrid />
                ...
                <CardContent>
                  ...
                  {benchmarkData.market_position && (
                    <div className="mt-4 p-3 bg-violet-50 rounded-lg">
                      <p className="text-xs font-medium text-violet-700 mb-1">Your Market Position</p>
                        ...
                        {adv.data && (
                          <div className="mt-2 flex gap-2">
                            {Object.entries(adv.data).map(([key, val]) => (
                              <Badge key={key} className="bg-emerald-100 text-emerald-700 border-0 text-xs">
                                {key}: {val}
                              </Badge>
                            ))}
                          </div>
            ...
            <Card>
              ...
              >
                <CardTitle className="text-sm flex items-center justify-between text-amber-700">
                  ...
                  {expandedSections.disadvantages ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                </CardTitle>
              </CardHeader>
                  ...
                  <div className="space-y-3">
                    {benchmarkData.competitive_disadvantages.map((dis, i) => (
                      <div key={i} className="border-l-4 border-amber-500 pl-3 py-2">
                        ...
                        {dis.gap && (
                          <p className="text-xs text-amber-700 mt-1">Gap: {dis.gap}</p>
                        )}
                        {dis.leader && (
                  ...
                  <div className="space-y-3">
                    {benchmarkData.strategic_recommendations.map((rec, i) => (
                      <div key={i} className="p-3 bg-violet-50 rounded-lg">
                        ...
                        {rec.expected_impact && (
                          <p className="text-xs text-violet-700 mt-2">Expected impact: {rec.expected_impact}</p>
                        )}
                      </div>
                    ))}
                  </div>
      ...
      {!benchmarkData && selectedCompetitors.length === 0 && (
        <Card className="p-8 text-center">
          ...
          <p className="text-sm text-gray-400">Compare performance across key metrics and get AI-powered insights</p>
        </Card>
      )}
    </div>
  );
