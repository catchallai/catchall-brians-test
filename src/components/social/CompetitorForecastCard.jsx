...
import { Loader2, Target, TrendingUp, TrendingDown, Eye, Swords, Shield, Lightbulb } from "lucide-react";
...
Be specific with timing, tactics, and actionable counter-moves.`,
  ...
  const marketShareData = forecast?.market_share_projection?.competitor_shares?.map(c => ({
    name: c.name,
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <Button 
            ...
            className="gap-2"
          >
            {isForecasting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Eye className="w-4 h-4" />}
        ...
        {!forecast ? (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4">
              <p className="text-sm text-gray-700 dark:text-gray-300">{forecast.executive_summary}</p>
            </div>

            {/* Market Share Projection */}
            {marketShareData.length > 0 && (
              <div>
                <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">30-Day Market Share Projection</h4>
                <div className="h-48">
                ...
                {forecast.market_share_projection?.change_from_current && (
                  <div className="mt-2 flex items-center gap-2 justify-center">
                    ...
                    <Badge className={forecast.market_share_projection.change_from_current >= 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}>
                      {forecast.market_share_projection.change_from_current >= 0 ? '+' : ''}{forecast.market_share_projection.change_from_current}%
                    </Badge>
                  </div>
                )}
              </div>
                  ...
                  {forecast.competitor_forecasts.map((comp, idx) => (
                    <div key={idx} className="border border-gray-100 dark:border-gray-700 rounded-xl p-4">
                      <div className="flex items-center justify-between mb-3">
                        <span className="font-medium text-gray-900 dark:text-white">{comp.competitor_name}</span>
                        <Badge className={comp.predicted_growth >= 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}>
                          {comp.predicted_growth >= 0 ? <TrendingUp className="w-3 h-3 mr-1" /> : <TrendingDown className="w-3 h-3 mr-1" />}
                      ...
                      {comp.likely_campaigns?.map((campaign, cIdx) => (
                        <div key={cIdx} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 mb-2">
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{campaign.campaign_type}</span>
                            <span className="text-xs text-gray-500">{campaign.expected_timing}</span>
                          </div>
                          <p className="text-xs text-gray-500 mt-1">{campaign.predicted_impact}</p>
                          <Badge className="mt-2 text-xs" variant="outline">{campaign.confidence}% confidence</Badge>
                        </div>
                      ))}
                      {comp.vulnerability_windows?.length > 0 && (
                        <div className="mt-2">
                          <span className="text-xs text-amber-600">⚡ Vulnerability windows: {comp.vulnerability_windows.join(', ')}</span>
                        </div>
                      )}
                    </div>
                  ))}
                  ...
                  {forecast.threat_assessment.map((threat, idx) => (
                    <div key={idx} className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-medium text-gray-900 dark:text-white">{threat.threat}</span>
                        <Badge className={getSeverityColor(threat.severity)}>{threat.severity}</Badge>
                      </div>
                      <p className="text-xs text-gray-500">From: {threat.source} • Timeline: {threat.timeline}</p>
                      <p className="text-xs text-blue-600 mt-2">Counter: {threat.counter_strategy}</p>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {forecast.recommended_counter_strategies.map((strategy, idx) => (
                    <div key={idx} className="bg-amber-50 dark:bg-amber-900/20 rounded-lg p-3">
                      ...
                      <p className="text-xs text-gray-500 mt-1">Target: {strategy.target_competitor}</p>
                      <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">{strategy.implementation}</p>
                      <Badge className="mt-2 bg-emerald-100 text-emerald-700 text-xs">{strategy.expected_outcome}</Badge>
                    </div>
                  ))}
                </div>
              </div>
