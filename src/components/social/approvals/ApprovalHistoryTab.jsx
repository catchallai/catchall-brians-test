...
import { Search, CheckCircle2, XCircle, RotateCcw, Send, Eye, ThumbsUp, Clock, Calendar } from "lucide-react";
...
export default function ApprovalHistoryTab({ posts, currentUser, statusLabels }) {
  ...
  const allEvents = posts
    .flatMap(post =>
      (post.workflow_history || []).map(entry => ({ ...entry, post }))
    )
  ...
  const completedPosts = posts.filter(p => ['approved', 'published', 'rejected'].includes(p.status));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Approved', value: posts.filter(p => ['approved','published'].includes(p.status)).length, color: 'text-green-600' },
          { label: 'Rejected', value: posts.filter(p => p.status === 'rejected').length, color: 'text-red-600' },
          { label: 'Changes Requested', value: posts.filter(p => p.status === 'changes_requested').length, color: 'text-orange-600' },
        ].map(stat => (
          <div key={stat.label} className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-4 text-center">
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search history…" className="pl-9 h-9 text-sm" />
        </div>
        ...
        {allEvents.length === 0 ? (
          ...
          <div className="divide-y divide-gray-50 dark:divide-gray-800">
            {allEvents.map((event, i) => {
              const cfg = ACTION_ICONS[event.action] || { icon: Clock, color: 'text-gray-400 bg-gray-50' };
              const Icon = cfg.icon;
              const postStatus = statusLabels[event.post?.status] || { label: event.post?.status, color: 'bg-gray-100 text-gray-600' };

              return (
                <div key={i} className="flex items-start gap-4 p-4 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
