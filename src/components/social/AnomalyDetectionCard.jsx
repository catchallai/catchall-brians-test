...
export default function AnomalyDetectionCard({ anomalies }) {
    ...
    return (
      <Card className="border-0 shadow-sm">
        <CardContent className="p-8 text-center">
          ...
          <h3 className="font-semibold text-gray-900 dark:text-white mb-2">No Anomalies Detected</h3>
  ...
  const getTypeIcon = (type) => {
    switch(type) {
      case 'engagement_drop': return TrendingUp;
      case 'follower_spike': return Activity;
      case 'sentiment_shift': return AlertTriangle;
      case 'posting_pattern': return Zap;
    ...
    <Card className="border-0 shadow-sm">
      ...
      <CardContent className="space-y-3">
        {anomalies.map((anomaly, idx) => {
          ...
          return (
            <div key={idx} className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                  ...
                  {anomaly.detected_value !== undefined && anomaly.baseline_value !== undefined && (
                    <div className="flex gap-4 text-xs text-gray-500 mb-2">
                      ...
                      <span className={anomaly.change > 0 ? 'text-emerald-600' : 'text-red-600'}>
                        {anomaly.change > 0 ? '+' : ''}{anomaly.change}% change
                      </span>
                    </div>
                  )}
