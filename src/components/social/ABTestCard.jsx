...
export default function ABTestCard({ test, onStart, onComplete, onClick }) {
  ...
  return (
    <Card className="p-4 border-0 shadow-sm hover:shadow-md transition-all cursor-pointer" onClick={onClick}>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className={`p-2 rounded-lg border ${test.winner === 'a' ? 'border-emerald-300 bg-emerald-50' : 'border-gray-200'}`}>
          ...
          <p className="text-xs text-gray-600 line-clamp-2">{test.variant_a?.content || 'No content'}</p>
        </div>
        <div className={`p-2 rounded-lg border ${test.winner === 'b' ? 'border-emerald-300 bg-emerald-50' : 'border-gray-200'}`}>
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs font-medium text-gray-500">Variant B</span>
            {test.winner === 'b' && <Trophy className="w-3 h-3 text-emerald-500" />}
          </div>
          <p className="text-xs text-gray-600 line-clamp-2">{test.variant_b?.content || 'No content'}</p>
        </div>
      </div>
      ...
      <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
        ...
        {test.status === 'running' && (
          <Button size="sm" variant="outline" className="flex-1 gap-1" onClick={() => onComplete(test)}>
