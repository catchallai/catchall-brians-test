      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto">
          ...
          <div className="space-y-2">
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Select Approved Copy</p>
            <div className="grid grid-cols-1 gap-2 max-h-32 overflow-y-auto border border-gray-100 dark:border-slate-700 rounded-lg p-2">
              ...
              {approvedCopy.map(copy => (
                <button key={copy.id} onClick={() => setSelectedCopy(copy)}
                  ...
                  <div className="flex items-start justify-between gap-1">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-gray-800 dark:text-white">{copy.title}</p>
                      <p className="text-xs text-gray-400 truncate">{copy.copy_type?.replace(/_/g, ' ')}</p>
                    </div>
                    <Checkbox checked={selectedCopy?.id === copy.id} className="pointer-events-none" />
                  </div>
                </button>
              ))}
            </div>
          ...
          {/* Template Selection */}
          <div className="space-y-2">
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Select Approved Template</p>
            <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto border border-gray-100 dark:border-slate-700 rounded-lg p-2">
              {approvedTemplates.length === 0 && (
                <p className="text-xs text-gray-400 col-span-2 py-4 text-center">No approved templates available</p>
              )}
              {approvedTemplates.map(tpl => (
                <button key={tpl.id} onClick={() => setSelectedTemplate(tpl)}
                  ...
                  {tpl.preview_url ? (
                    <img src={tpl.preview_url} alt={tpl.title} className="w-full h-20 object-cover" />
                  ) : (
