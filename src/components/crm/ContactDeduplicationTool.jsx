...
export default function ContactDeduplicationTool({ contacts, onDeduped }) {
  ...
  const handleMerge = async () => {
    ...
    try {
      ...
      toast.success(`Merged ${mergeCandidate.first_name} ${mergeCandidate.last_name} into primary contact`);
      onDeduped?.();
                ...
                <div key={idx} className="p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">{dup.contact.email}</p>
                      <Badge variant="outline" className="mt-1 text-xs">Duplicate</Badge>
                    </div>
            ...
            {mergeCandidate && (
              <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg space-y-3">
                <p className="text-sm font-medium text-gray-900 dark:text-white">
                  Merging: <span className="text-blue-600">{mergeCandidate.first_name} {mergeCandidate.last_name}</span>
                </p>
                    ...
                    {contacts
                      .filter(c => c.id !== mergeCandidate.id && 
                        (c.email === mergeCandidate.email || 
                         (c.first_name === mergeCandidate.first_name && c.last_name === mergeCandidate.last_name)))
                      .map(c => (
