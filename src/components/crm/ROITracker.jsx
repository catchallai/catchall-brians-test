  ...
  const calculateCampaignROI = () => {
    return campaigns.map(campaign => {
      ...
      return {
        ...
        costPerLead,
        conversionRate: leads > 0 ? (campaignDeals.filter(d => d.stage === 'closed_won').length / leads) * 100 : 0,
      };
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-4 gap-3">
          <div className="text-center p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg">
            ...
            <p className="text-xl font-bold text-emerald-600">${(totalRevenue / 1000).toFixed(1)}K</p>
          ...
          <div className="text-center p-3 bg-violet-50 dark:bg-violet-900/20 rounded-lg">
            ...
            <p className={`text-xl font-bold ${overallROI >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
        ...
        {chartData.length > 0 && (
          <div>
            <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Revenue vs Cost by Campaign</p>
            <div className="h-40">
        ...
        {/* Campaign Breakdown */}
        <div>
          <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Campaign Performance</p>
          <div className="space-y-3 max-h-48 overflow-y-auto">
            {campaignROI.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-4">No campaign data with budget/revenue</p>
            ) : (
              campaignROI.slice(0, 5).map((campaign) => (
                <div key={campaign.id} className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Badge className={campaign.roi >= 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}>
