...
export default function ABTestModal({ open, onClose, test, accounts, onSave, onGenerateVariant, isLoading, isGenerating }) {
  ...
  const handleGenerateVariant = async (variant) => {
    const baseContent = variant === 'variant_a' ? formData.variant_b.content : formData.variant_a.content;
            ...
            <TabsContent value="variant_a" className="space-y-3">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addHashtag('variant_a', newHashtagA, setNewHashtagA))}
                />
                <Button type="button" variant="outline" onClick={() => addHashtag('variant_a', newHashtagA, setNewHashtagA)}>Add</Button>
              </div>
                ...
                <div className="flex gap-1 flex-wrap">
                  {formData.variant_a.hashtags.map((tag, i) => (
                    <Badge key={i} variant="outline" className="gap-1">
                      #{tag}
                      <X className="w-3 h-3 cursor-pointer" onClick={() => removeHashtag('variant_a', tag)} />
                    </Badge>
                  ))}
                </div>
              )}
            ...
            <TabsContent value="variant_b" className="space-y-3">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  placeholder="Add hashtag"
                  onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addHashtag('variant_b', newHashtagB, setNewHashtagB))}
                />
                <Button type="button" variant="outline" onClick={() => addHashtag('variant_b', newHashtagB, setNewHashtagB)}>Add</Button>
              </div>
              {formData.variant_b.hashtags?.length > 0 && (
                <div className="flex gap-1 flex-wrap">
                  {formData.variant_b.hashtags.map((tag, i) => (
                    <Badge key={i} variant="outline" className="gap-1">
                      #{tag}
                      <X className="w-3 h-3 cursor-pointer" onClick={() => removeHashtag('variant_b', tag)} />
                    </Badge>
                  ))}
                </div>
              )}
