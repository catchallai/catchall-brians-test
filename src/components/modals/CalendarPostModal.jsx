...
function PlatformPreviewPanel({ platform, caption, imageUrl, videoUrl }) {
	...
	const truncated =
		caption.length > p.limit ? caption.slice(0, p.limit) + '…' : caption;
	...
	return (
		<div className='space-y-2'>
			<div className='flex items-center gap-1.5 text-xs text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg px-3 py-2'>
				...
				<span>
					Best times for <strong>{primaryPlatform}</strong> based on typical
					audience activity:
				</span>
			</div>
		...
		<div className='space-y-3 pt-1'>
			<div className='flex items-center gap-2'>
				<Select
					...
					onValueChange={(v) =>
						setFormData((f) => ({ ...f, recurrence_type: v }))
					}>
			...
			<div>
				...
				<input
					...
					onChange={(e) =>
						setFormData((f) => ({ ...f, recurrence_end_date: e.target.value }))
					}
...
export default function CalendarPostModal({
	...
	const handleSubmit = (status) => {
		...
		const finalStatus =
			isAdmin && requireApproval && status === 'approved'
				? 'pending_approval'
				: status;
		onSave({
			...formData,
	...
	const activePlatform =
		PLATFORMS.find((p) => p.id === previewPlatform) || PLATFORMS[3];
