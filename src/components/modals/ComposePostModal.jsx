...
export default function ComposePostModal({
		...
		const platforms = [
			...new Set(
				selectedAccounts.map(
					(id) => accounts.find((a) => a.id === id)?.platform,
				),
			),
		].filter(Boolean);
	...
	const selectedPlatforms = [
		...new Set(
			selectedAccounts.map((id) => accounts.find((a) => a.id === id)?.platform),
		),
	].filter(Boolean);
						...
						<div className='space-y-2'>
							...
							{accounts.length === 0 ? (
								<p className='text-sm text-gray-400 p-4 bg-gray-50 rounded-lg text-center'>
									No social accounts connected. Add accounts in Social Media
									page first.
								</p>
							) : (
					...
					<TabsContent
						...
						<div className='p-4 bg-gray-50 rounded-lg space-y-2'>
							...
							<p className='text-sm text-gray-600'>
								...
								{selectedAccounts.length !== 1 ? 's' : ''} across{' '}
