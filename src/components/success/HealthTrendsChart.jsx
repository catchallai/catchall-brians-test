...
export default function HealthTrendsChart({ healthScores }) {
  ...
  const trendData = React.useMemo(() => {
    ...
    healthScores.forEach(score => {
      const date = new Date(score.last_calculated || score.created_date).toISOString().split('T')[0];
      if (!grouped[date]) {
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={trendData}>
            ...
            <Line type="monotone" dataKey="at_risk" stroke="#f59e0b" strokeWidth={2} name="At Risk" />
