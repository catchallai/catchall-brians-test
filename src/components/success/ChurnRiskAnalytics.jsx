...
export default function ChurnRiskAnalytics({ healthScores, onboardings, interactions, opportunities }) {
  ...
  const calculateChurnRisk = (customer) => {
    ...
    const recentInteractions = interactions.filter(i => i.contact_id === customer.contact_id)
      .filter(i => {
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-lg">
            ...
            <p className="text-2xl font-bold text-red-700 dark:text-red-300">{highRiskCustomers.length}</p>
          </div>
          <div className="p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
            <p className="text-xs text-amber-600 dark:text-amber-400 mb-1">Medium Risk</p>
            <p className="text-2xl font-bold text-amber-700 dark:text-amber-300">{mediumRiskCustomers.length}</p>
          </div>
          <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg">
            <p className="text-xs text-emerald-600 dark:text-emerald-400 mb-1">Low Risk</p>
            <p className="text-2xl font-bold text-emerald-700 dark:text-emerald-300">{lowRiskCustomers.length}</p>
          </div>
