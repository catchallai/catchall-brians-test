...
export default function EmailCampaignCard({ emailCampaign, template, onClick }) {
  const openRate = emailCampaign.total_sent > 0 
    ? ((emailCampaign.total_opened || 0) / emailCampaign.total_sent) * 100 
    : 0;
  const clickRate = emailCampaign.total_opened > 0 
        ...
        <>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs text-gray-500">
              <span>Open Rate</span>
              <span>{emailCampaign.total_opened || 0} / {emailCampaign.total_sent || 0}</span>
            </div>
