...
import { 
  Globe, TrendingUp, TrendingDown, AlertTriangle, Link2, 
  BarChart3, Minus 
} from "lucide-react";
...
export default function BacklinkAnalytics({ backlinks }) {
  ...
  const toxicTrend = useMemo(() => {
    const toxicLinks = backlinks.filter(b => b.is_toxic || (b.domain_authority && b.domain_authority < 10));
  ...
  const daDistribution = useMemo(() => {
    ...
    return ranges.map(range => ({
      ...range,
      value: backlinks.filter(b => 
        b.domain_authority >= range.min && b.domain_authority <= range.max
      ).length
    })).filter(r => r.value > 0);
  }, [backlinks]);
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{linkVelocity.current}</p>
                <p className="text-xs text-gray-500">Links This Week</p>
              </div>
            </div>
            <Badge className={`text-xs ${
              linkVelocity.trend > 0 ? 'bg-emerald-100 text-emerald-700' :
              linkVelocity.trend < 0 ? 'bg-red-100 text-red-700' :
              'bg-gray-100 text-gray-600'
            }`}>
              {linkVelocity.trend > 0 ? <TrendingUp className="w-3 h-3 mr-1" /> :
               linkVelocity.trend < 0 ? <TrendingDown className="w-3 h-3 mr-1" /> :
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`p-2.5 rounded-xl ${
                parseFloat(toxicTrend.current) > 10 ? 'bg-red-100 dark:bg-red-900/30' : 'bg-emerald-100 dark:bg-emerald-900/30'
              }`}>
              ...
              <div>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{toxicTrend.current}%</p>
                <p className="text-xs text-gray-500">Toxic Score</p>
              </div>
            </div>
            <Badge className={`text-xs ${
              toxicTrend.trend < 0 ? 'bg-emerald-100 text-emerald-700' :
              toxicTrend.trend > 0 ? 'bg-red-100 text-red-700' :
              'bg-gray-100 text-gray-600'
            }`}>
