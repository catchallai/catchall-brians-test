...
export default function QuickActionsGrid({ keywords, backlinks }) {
  const getCount = (page) => {
    switch(page) {
      ...
      case 'Backlinks': return backlinks?.length || 0;
      default: return null;
    }
  };
      ...
      {actions.map((action) => {
        ...
        return (
          <Link key={action.page} to={createPageUrl(action.page)}>
            <div className={`group relative overflow-hidden rounded-xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-4 transition-all duration-300 ${action.bgHover} hover:shadow-md hover:border-transparent cursor-pointer`}>
              <div className="flex flex-col items-center text-center gap-2">
                <div className={`p-2.5 rounded-xl bg-gradient-to-br ${action.gradient} text-white shadow-md group-hover:scale-110 group-hover:shadow-lg transition-all`}>
