...
import { 
  FileText, Target, TrendingUp, AlertCircle, CheckCircle, 
  ChevronDown, ChevronUp, Lightbulb, Hash
} from 'lucide-react';
...
export default function ContentOptimizationCard({ data, keywords = [] }) {
  ...
  const pages = data?.pages || [
    ...
    },
    {
      ...
      issues: [
        { type: 'info', message: 'Consider adding FAQ schema for pricing questions' },
      ],
      suggestions: [
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-4 gap-3">
          <div className="text-center p-3 bg-violet-50 dark:bg-violet-900/20 rounded-lg">
            <p className="text-2xl font-bold text-violet-600 dark:text-violet-400">{overallStats.avgScore}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">Avg Score</p>
          </div>
          <div className="text-center p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
            <p className="text-2xl font-bold text-amber-600 dark:text-amber-400">{overallStats.totalIssues}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">Total Issues</p>
          </div>
          <div className="text-center p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg">
            <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{overallStats.optimizedPages}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">Optimized</p>
          </div>
          <div className="text-center p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
            <p className="text-2xl font-bold text-red-600 dark:text-red-400">{overallStats.needsWork}</p>
            ...
            <div key={idx} className="border border-gray-100 dark:border-gray-700 rounded-xl overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className={`w-12 h-12 rounded-lg ${getScoreBg(page.score)} flex items-center justify-center`}>
                    <span className={`text-lg font-bold ${getScoreColor(page.score)}`}>{page.score}</span>
                  </div>
                  <div className="text-left min-w-0">
                    <p className="font-medium text-gray-900 dark:text-white truncate">{page.title}</p>
                ...
                <div className="p-4 pt-0 space-y-4 border-t border-gray-100 dark:border-gray-700">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="p-2 bg-gray-50 dark:bg-gray-900 rounded-lg text-center">
                      <p className="text-sm font-medium text-gray-900 dark:text-white">{page.keywordDensity}%</p>
                      <p className="text-xs text-gray-500">Keyword Density</p>
                    </div>
                    <div className="p-2 bg-gray-50 dark:bg-gray-900 rounded-lg text-center">
                      <p className="text-sm font-medium text-gray-900 dark:text-white">{page.wordCount}</p>
                      <p className="text-xs text-gray-500">Word Count</p>
                    </div>
                    <div className="p-2 bg-gray-50 dark:bg-gray-900 rounded-lg text-center">
                      <p className="text-sm font-medium text-gray-900 dark:text-white">{page.readability}</p>
