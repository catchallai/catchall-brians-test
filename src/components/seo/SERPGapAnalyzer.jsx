          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-4 gap-3">
              <div className="text-center p-3 bg-violet-50 rounded-lg">
                <p className="text-2xl font-bold text-violet-600">{results.summary?.total_gap_keywords || 0}</p>
                <p className="text-xs text-gray-500">Keyword Gaps</p>
              </div>
              <div className="text-center p-3 bg-emerald-50 rounded-lg">
                <p className="text-2xl font-bold text-emerald-600">{results.summary?.quick_wins || 0}</p>
                <p className="text-xs text-gray-500">Quick Wins</p>
              </div>
              <div className="text-center p-3 bg-blue-50 rounded-lg">
                <p className="text-2xl font-bold text-blue-600">{formatNumber(results.summary?.potential_traffic)}</p>
                <p className="text-xs text-gray-500">Traffic Potential</p>
              </div>
              <div className="text-center p-3 bg-amber-50 rounded-lg">
                <p className="text-2xl font-bold text-amber-600">{results.summary?.content_gaps || 0}</p>
            ...
            <Tabs defaultValue="gaps" className="space-y-4">
              ...
              <TabsContent value="gaps" className="space-y-2">
                {results.keyword_gaps?.slice(0, 15).map((gap, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 bg-white border rounded-lg hover:bg-gray-50">
                    <div className="flex-1">
                      <p className="font-medium text-gray-900">{gap.keyword}</p>
                      <div className="flex items-center gap-3 text-xs text-gray-500 mt-1">
                        <span>Vol: {formatNumber(gap.search_volume)}</span>
                        <Badge className={getDifficultyColor(gap.difficulty)}>KD: {gap.difficulty}</Badge>
                        <span>{gap.competitor} #{gap.competitor_position}</span>
                        <Badge variant="outline">{gap.intent}</Badge>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="text-right">
                        <p className="text-sm font-medium text-violet-600">{gap.opportunity_score}/100</p>
                        <p className="text-xs text-gray-400">Opportunity</p>
                      </div>
                      <Button 
                        size="sm" 
                        variant="ghost"
                        onClick={() => onAddKeyword?.({ keyword: gap.keyword, search_volume: gap.search_volume, difficulty: gap.difficulty })}
                      >
                  ...
                  <div key={idx} className="flex items-center justify-between p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-500 mt-1">
                        ...
                        <Badge className="bg-emerald-100 text-emerald-700">KD: {win.difficulty}</Badge>
                      </div>
                    </div>
                    <Button 
                      size="sm" 
                      className="bg-emerald-600 hover:bg-emerald-700"
                      onClick={() => onAddKeyword?.({ keyword: win.keyword, search_volume: win.search_volume, difficulty: win.difficulty })}
                    >
                  ...
                  <div key={idx} className="p-3 bg-white border rounded-lg">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-gray-500 mt-1">{formatNumber(gap.traffic_potential)} potential traffic</p>
                      </div>
                    </div>
                  </div>
                ))}
                  ...
                  <div key={idx} className="p-4 bg-white border rounded-lg">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">{formatNumber(comp.estimated_traffic)} monthly organic traffic</p>
                      </div>
                    </div>
                  ...
                  <div key={idx} className="flex items-start gap-3 p-3 bg-white border rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 mt-1 text-xs">
                        <span className="text-gray-500">Impact: <span className="text-gray-900">{rec.expected_impact}</span></span>
                        <span className="text-gray-500">Effort: <span className="text-gray-900">{rec.effort}</span></span>
                      </div>
                    </div>
                    <Badge className={
