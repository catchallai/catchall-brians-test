...
export default function BacklinkItem({ backlink, onDisavow }) {
  ...
  const isBadBacklink = backlink.is_toxic || backlink.domain_authority < 10 || backlink.status === 'broken';
  ...
  return (
    <div className={`flex items-start gap-4 p-4 rounded-xl bg-white dark:bg-gray-800 border transition-all ${
      backlink.status === 'disavowed' ? 'border-gray-200 dark:border-gray-700 opacity-60' :
      isBadBacklink ? 'border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/10' : 
      'border-gray-100 dark:border-gray-700 hover:border-gray-200'
    }`}>
      <div className={`p-2 rounded-lg ${isBadBacklink && backlink.status !== 'disavowed' ? 'bg-red-100' : 'bg-violet-50 dark:bg-violet-900/30'}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <h4 className={`font-medium truncate ${backlink.status === 'disavowed' ? 'text-gray-400 line-through' : 'text-gray-900 dark:text-white'}`}>
        ...
        <div className="flex items-center gap-4 text-xs text-gray-400">
          <span>DA: <span className={`font-medium ${backlink.domain_authority < 10 ? 'text-red-600' : 'text-gray-600 dark:text-gray-300'}`}>
            {backlink.domain_authority || '-'}
          </span></span>
          {backlink.anchor_text && (
            <span>Anchor: <span className="font-medium text-gray-600 dark:text-gray-300">"{backlink.anchor_text}"</span></span>
          )}
