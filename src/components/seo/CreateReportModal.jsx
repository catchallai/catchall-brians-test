...
export default function CreateReportModal({ open, onClose, onSave, websites, isLoading }) {
  ...
  const handleSave = () => {
    const nextRun = form.schedule === 'weekly' 
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div>
            <Label>Website</Label>
            <Select value={form.website_id} onValueChange={(v) => setForm({ ...form, website_id: v })}>
              <SelectTrigger>
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex items-center justify-between">
                ...
                <Switch checked={form.include_traffic} onCheckedChange={(v) => setForm({ ...form, include_traffic: v })} />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm">Rankings</span>
                <Switch checked={form.include_rankings} onCheckedChange={(v) => setForm({ ...form, include_rankings: v })} />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm">Backlinks</span>
                <Switch checked={form.include_backlinks} onCheckedChange={(v) => setForm({ ...form, include_backlinks: v })} />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm">Trends</span>
                <Switch checked={form.include_trends} onCheckedChange={(v) => setForm({ ...form, include_trends: v })} />
              </div>
            </div>
          </div>
