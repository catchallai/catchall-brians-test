...
import {
  Plus, X, ChevronRight, Target, Megaphone, Globe, Mail,
  Smartphone, Radio, LayoutGrid, Loader2
} from "lucide-react";
...
export default function CampaignBriefTab({ currentUser }) {
  ...
  const toggleCopyOnBrief = (brief, copyId) => {
    ...
    const updated = current.includes(copyId) ? current.filter(i => i !== copyId) : [...current, copyId];
  ...
  const toggleTemplateOnBrief = (brief, tplId) => {
    ...
    const updated = current.includes(tplId) ? current.filter(i => i !== tplId) : [...current, tplId];
    ...
    <div className="flex gap-4 h-full">
      ...
      <div className="w-72 flex-shrink-0 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" onClick={() => setShowForm(true)} className="bg-violet-600 hover:bg-violet-700 text-white gap-1">
        ...
        {showForm && (
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-3 space-y-2">
            <div className="flex items-center justify-between mb-1">
              ...
              <button onClick={() => setShowForm(false)}><X className="w-3.5 h-3.5 text-gray-400" /></button>
            </div>
            <Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Title e.g. March Multi-Channel" className="text-sm h-8" />
            <Input value={form.month} onChange={e => setForm(f => ({ ...f, month: e.target.value }))} placeholder="Period e.g. 2026-03" className="text-sm h-8" />
            <Input value={form.focal_point} onChange={e => setForm(f => ({ ...f, focal_point: e.target.value }))} placeholder="Focal point / theme" className="text-sm h-8" />
            <Textarea value={form.objective} onChange={e => setForm(f => ({ ...f, objective: e.target.value }))} placeholder="Campaign objective..." className="text-sm min-h-[60px] resize-none" />
            <div className="flex gap-2 justify-end pt-1">
              <Button size="sm" variant="outline" onClick={() => setShowForm(false)}>Cancel</Button>
              <Button size="sm" onClick={() => createMutation.mutate({ ...form, status: 'draft', owner_email: currentUser?.email, owner_name: currentUser?.full_name, workflow_history: [] })}
                disabled={!form.title || createMutation.isPending} className="bg-violet-600 hover:bg-violet-700 text-white">
        ...
        <div className="space-y-2">
          ...
          {briefs.map(brief => {
            ...
            return (
              <button key={brief.id} onClick={() => setSelected(isSelected ? null : brief)}
                ...
                <div className="flex items-start justify-between gap-1">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-gray-800 dark:text-white truncate">{brief.title}</p>
                    <p className="text-xs text-gray-400 mt-0.5">{brief.month}</p>
                    {brief.focal_point && <p className="text-xs text-violet-600 dark:text-violet-400 mt-0.5 truncate">{brief.focal_point}</p>}
                  </div>
          ...
          {!isLoading && briefs.length === 0 && (
            <div className="text-xs text-gray-400 text-center py-8">No briefs yet. Create your first.</div>
          )}
        </div>
      </div>
        ...
        <div className="flex-1 min-w-0 space-y-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 space-y-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-gray-500">{selected.month} · {selected.focal_point}</p>
              </div>
              <div className="flex items-center gap-2">
                <Badge className={`${(STATUS_CONFIG[selected.status] || STATUS_CONFIG.draft).color}`}>
            ...
            {/* Channel Drivers */}
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Channel Drivers</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {(selected.drivers || CHANNELS.map(c => ({ channel: c, focus: '', notes: '' }))).map(driver => {
                  const Icon = CHANNEL_ICONS[driver.channel] || LayoutGrid;
                  return (
                    <div key={driver.channel} className="bg-gray-50 dark:bg-slate-700/50 rounded-lg p-3">
                      <div className="flex items-center gap-1.5 mb-1.5">
                        <Icon className="w-3.5 h-3.5 text-violet-500" />
                        <p className="text-xs font-bold text-gray-700 dark:text-gray-200">{driver.channel}</p>
                      </div>
                      {driver.focus ? (
                      ...
                      {driver.notes && <p className="text-xs text-gray-400 mt-0.5">{driver.notes}</p>}
                    </div>
                  );
                })}
              </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
            <p className="text-sm font-bold text-gray-800 dark:text-white mb-3">
              Approved Copy Pool
              <span className="ml-2 text-xs font-normal text-gray-400">({(selected.approved_copy_ids || []).length} linked)</span>
            </p>
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {approvedCopy.length === 0 && (
                <p className="text-xs text-gray-400">No approved copy yet. Approve copy in the Copy Pool tab first.</p>
              )}
              {approvedCopy.map(copy => {
                const linked = (selected.approved_copy_ids || []).includes(copy.id);
                return (
                  <div key={copy.id} className={`flex items-center justify-between p-2.5 rounded-lg border transition-all ${linked ? 'border-green-200 bg-green-50 dark:bg-green-900/20 dark:border-green-800' : 'border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-700/30'}`}>
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-semibold text-gray-700 dark:text-gray-200 truncate">{copy.title}</p>
                      <p className="text-xs text-gray-400 truncate">{copy.copy_type?.replace(/_/g, ' ')} · {(copy.channels || []).join(', ')}</p>
                    </div>
                    <button onClick={() => toggleCopyOnBrief(selected, copy.id)}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
            <p className="text-sm font-bold text-gray-800 dark:text-white mb-3">
              ...
              <span className="ml-2 text-xs font-normal text-gray-400">({(selected.approved_template_ids || []).length} linked)</span>
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-64 overflow-y-auto">
              ...
              {approvedTemplates.map(tpl => {
                ...
                return (
                  <button key={tpl.id} onClick={() => toggleTemplateOnBrief(selected, tpl.id)}
                    ...
                    {tpl.preview_url ? (
                      <img src={tpl.preview_url} alt={tpl.title} className="w-full h-16 object-cover" />
                    ) : (
                    ...
                    <div className="p-1.5 bg-white dark:bg-slate-800">
                      <p className="text-xs font-medium text-gray-700 dark:text-gray-200 truncate">{tpl.title}</p>
