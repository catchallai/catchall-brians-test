...
import { Plus, Search, Image, X, ExternalLink, ChevronRight } from "lucide-react";
...
const TEMPLATE_TYPES = ['social_post', 'story', 'reel_cover', 'ad_banner', 'email_header', 'newsletter_layout', 'website_hero', 'carousel_slide', 'other'];
const AESTHETICS = ['minimal', 'bold', 'editorial', 'lifestyle', 'corporate', 'playful', 'luxury', 'documentary'];
...
export default function GraphicTemplatePoolTab({ currentUser }) {
  ...
  const [form, setForm] = useState({ title: '', template_type: 'social_post', aesthetic_style: 'minimal', preview_url: '', file_url: '', dimensions: '', channels: [] });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (newItem) => {
      ...
      setShowForm(false);
      setForm({ title: '', template_type: 'social_post', aesthetic_style: 'minimal', preview_url: '', file_url: '', dimensions: '', channels: [] });
      setSelected(newItem);
    },
  });
  ...
  const pendingCount = items.filter(i => ['pending_review', 'pending_brand_approval'].includes(i.status)).length;
    ...
    <div className="flex gap-4 h-full">
      <div className="flex-1 min-w-0 space-y-3">
        ...
        <div className="flex gap-3 items-center">
          ...
          <div className="ml-auto">
            <Button size="sm" onClick={() => setShowForm(true)} className="bg-violet-600 hover:bg-violet-700 text-white gap-1.5">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Select value={typeFilter} onValueChange={setTypeFilter}>
            <SelectTrigger className="h-8 w-40 text-sm"><SelectValue placeholder="Type" /></SelectTrigger>
            <SelectContent>
        ...
        {showForm && (
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <p className="font-semibold text-sm text-gray-800 dark:text-white">New Graphic Template</p>
              <button onClick={() => setShowForm(false)}><X className="w-4 h-4 text-gray-400" /></button>
            </div>
            <Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Template name" className="text-sm" />
            <div className="flex gap-2">
              ...
              <Select value={form.aesthetic_style} onValueChange={v => setForm(f => ({ ...f, aesthetic_style: v }))}>
                <SelectTrigger className="h-8 text-sm flex-1"><SelectValue /></SelectTrigger>
                <SelectContent>{AESTHETICS.map(a => <SelectItem key={a} value={a}>{a}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <Input value={form.preview_url} onChange={e => setForm(f => ({ ...f, preview_url: e.target.value }))} placeholder="Preview image URL" className="text-sm" />
            <Input value={form.file_url} onChange={e => setForm(f => ({ ...f, file_url: e.target.value }))} placeholder="Source file URL (Canva / Figma / Drive)" className="text-sm" />
            <Input value={form.dimensions} onChange={e => setForm(f => ({ ...f, dimensions: e.target.value }))} placeholder="Dimensions e.g. 1080x1080" className="text-sm" />
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button size="sm" onClick={() => createMutation.mutate({ ...form, status: 'draft', submitted_by: currentUser?.email, submitted_by_name: currentUser?.full_name, workflow_history: [] })}
                disabled={!form.title} className="bg-violet-600 hover:bg-violet-700 text-white">Save Draft</Button>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {isLoading && <div className="col-span-2 text-center py-8 text-gray-400 text-sm">Loading...</div>}
          {filtered.map(item => {
            ...
            return (
              <button key={item.id} onClick={() => setSelected(isSelected ? null : item)}
                ...
                <div className="p-3 bg-white dark:bg-slate-800">
                  ...
                  <p className="text-xs text-gray-400 mt-1">{item.template_type?.replace(/_/g, ' ')} · {item.aesthetic_style}</p>
                  {item.dimensions && <p className="text-xs text-gray-300 dark:text-gray-600">{item.dimensions}</p>}
                  {item.file_url && (
          ...
          {!isLoading && filtered.length === 0 && (
            <div className="col-span-2 text-center py-12 text-gray-400 text-sm">No templates found.</div>
          )}
        </div>
      </div>
      ...
      {selected && (
        <div className="w-72 flex-shrink-0 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 space-y-4 self-start sticky top-4">
          <div className="flex items-center justify-between">
            <p className="text-sm font-bold text-gray-800 dark:text-white truncate">{selected.title}</p>
            <button onClick={() => setSelected(null)}><X className="w-4 h-4 text-gray-400" /></button>
          </div>
          {selected.preview_url && (
            <img src={selected.preview_url} alt="" className="w-full rounded-lg object-cover max-h-36" />
          )}
