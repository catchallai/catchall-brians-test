...
import {
  Plus, Search, FileText, Filter, ChevronRight, X, Tag,
  CheckCircle2, Clock, RotateCcw, ShieldCheck, XCircle
} from "lucide-react";
...
const COPY_TYPES = ['headline', 'body_copy', 'cta', 'caption', 'subject_line', 'ad_copy', 'email_body', 'newsletter_intro', 'website_copy', 'other'];
const CHANNELS = ['Social', 'Email', 'Ads', 'Website', 'Newsletter', 'EDM', 'SMS'];
const TONES = ['professional', 'casual', 'inspirational', 'urgent', 'educational', 'playful', 'authoritative'];
...
export default function CopyPoolTab({ currentUser }) {
  ...
  const [form, setForm] = useState({ title: '', copy_type: 'caption', content: '', tone: 'professional', channels: [], tags: [] });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (newItem) => {
      ...
      setShowForm(false);
      setForm({ title: '', copy_type: 'caption', content: '', tone: 'professional', channels: [], tags: [] });
      setSelected(newItem);
    },
  });
      ...
      <div className="flex-1 min-w-0 space-y-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <div className="relative flex-1 min-w-40">
            ...
            <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search copy..." className="pl-8 h-8 text-sm" />
          </div>
          ...
          <Select value={typeFilter} onValueChange={setTypeFilter}>
            ...
            <SelectContent>
              ...
              {COPY_TYPES.map(t => <SelectItem key={t} value={t}>{t.replace(/_/g, ' ')}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        ...
        {showForm && (
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 space-y-3">
            ...
            <Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Title (internal reference)" className="text-sm" />
            <div className="flex gap-2">
              <Select value={form.copy_type} onValueChange={v => setForm(f => ({ ...f, copy_type: v }))}>
                <SelectTrigger className="h-8 text-sm flex-1"><SelectValue /></SelectTrigger>
                <SelectContent>{COPY_TYPES.map(t => <SelectItem key={t} value={t}>{t.replace(/_/g, ' ')}</SelectItem>)}</SelectContent>
              </Select>
            ...
            <Textarea value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))} placeholder="Copy text..." className="text-sm min-h-[100px] resize-none" />
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button size="sm" onClick={handleCreate} disabled={!form.title || !form.content} className="bg-violet-600 hover:bg-violet-700 text-white">Save Draft</Button>
            </div>
          </div>
        )}
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      <span className="text-xs bg-gray-50 dark:bg-gray-700 text-gray-500 px-1.5 py-0.5 rounded">{item.copy_type?.replace(/_/g, ' ')}</span>
                      {(item.channels || []).slice(0, 3).map(ch => (
                        <span key={ch} className="text-xs bg-violet-50 dark:bg-violet-900/30 text-violet-600 px-1.5 py-0.5 rounded">{ch}</span>
                      ))}
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                    <Badge className={`${cfg.color} text-xs`}>{cfg.label}</Badge>
                    <ChevronRight className={`w-3.5 h-3.5 text-gray-300 transition-transform ${isSelected ? 'rotate-90' : ''}`} />
                  </div>
                </div>
              </button>
            );
