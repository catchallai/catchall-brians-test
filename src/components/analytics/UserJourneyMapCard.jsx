          ...
          {showFilters && (
            <div className="flex flex-col sm:flex-row gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
              <div className="flex-1">
                <label className="text-xs font-medium text-gray-500 mb-1 block">Visitor Segment</label>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {selectedSegment !== 'all' && (
                <Badge variant="outline" className="text-xs gap-1">
                  ...
                  <button onClick={() => setSelectedSegment('all')} className="ml-1 hover:text-red-500">×</button>
                </Badge>
              )}
              {selectedEntryPoint !== 'all' && (
                <Badge variant="outline" className="text-xs">
                  {ENTRY_POINTS.find(e => e.id === selectedEntryPoint)?.label}
                  <button onClick={() => setSelectedEntryPoint('all')} className="ml-1 hover:text-red-500">×</button>
                </Badge>
              )}
            </div>
          )}
          ...
          <div className="space-y-6">
            ...
            <div>
              <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3 flex items-center gap-2">
                ...
                Conversion Funnel
                <Badge variant="outline" className="ml-2 text-xs">{segmentData.segmentLabel}</Badge>
              </h4>
              <div className="relative">
                {funnelData.map((stage, idx) => (
                  <div key={idx} className="flex items-center gap-3 mb-2">
                    <div className="w-20 text-xs text-gray-600 dark:text-gray-400 text-right">{stage.stage}</div>
                    <div className="flex-1 h-8 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden relative">
                      ...
                      <div className="absolute inset-0 flex items-center justify-between px-3">
                        <span className="text-xs font-medium text-white drop-shadow">{stage.visitors.toLocaleString()}</span>
                        <span className="text-xs text-gray-600 dark:text-gray-400">{stage.percentage}%</span>
                      </div>
                    </div>
                          ...
                          <TooltipTrigger>
                            <div className="w-12 text-center">
                              <span className="text-xs text-red-500 font-medium">
                                -{Math.round(100 - (funnelData[idx + 1].visitors / stage.visitors) * 100)}%
                              </span>
                            </div>
                          </TooltipTrigger>
                          <TooltipContent>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-violet-50 dark:bg-violet-900/20 rounded-xl p-3 text-center">
                <p className="text-xl font-bold text-violet-600">{segmentData.segment.conversionRate}%</p>
                <p className="text-xs text-gray-500">Conversion Rate</p>
              </div>
              <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-3 text-center">
                <p className="text-xl font-bold text-blue-600">{segmentData.segment.avgTouchpoints}</p>
                ...
                {commonPaths.map((path, idx) => (
                  <div key={idx} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                      ...
                      <div className="flex items-center gap-1 flex-wrap">
                        {path.path.map((step, sIdx) => (
                          <React.Fragment key={sIdx}>
                            <span className="text-xs bg-white dark:bg-gray-700 px-2 py-1 rounded font-medium">{step}</span>
                    ...
                    <div className="flex items-center gap-4 text-xs">
                      ...
                      <span className="text-emerald-600 font-medium">{path.conversions}% converts</span>
            ...
            <div>
              ...
              <div className="space-y-2">
                {dropOffPoints.map((drop, idx) => (
                  <div key={idx} className="flex items-center gap-3 p-2 bg-red-50 dark:bg-red-900/20 rounded-lg">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{drop.location}</p>
                      <p className="text-xs text-gray-500 truncate">{drop.reason}</p>
                    </div>
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger>
                          <Badge className="bg-emerald-100 text-emerald-700 text-xs shrink-0">Fix</Badge>
                        </TooltipTrigger>
