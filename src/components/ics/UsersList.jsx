...
export default function UsersList({ users, allPresence, darkMode, onSelectUser, currentUser, onViewProfile }) {
  ...
  return (
    <div className={`w-80 border-r ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-200'} flex flex-col`}>
      {/* Header */}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={`font-medium text-sm ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                        {user.full_name || 'User'} {isCurrentUser && <span className={`text-xs ml-1 ${darkMode ? 'text-violet-400' : 'text-violet-600'}`}>(You)</span>}
                      </div>
