  ...
  const addItemMutation = useMutation({
    mutationFn: async () => {
      if (!checklists.length) {
        ...
        const items = [...(checklist.items || []), { text: newItem, completed: false, order: checklist.items.length }];
          ...
          {checklist?.items?.map((item, idx) => (
            <div key={idx} className="flex items-center gap-2">
              <Checkbox
                ...
                onCheckedChange={() => toggleItemMutation.mutate({ checklistId: checklist.id, itemIndex: idx })}
              />
