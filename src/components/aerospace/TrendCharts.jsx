...
export default function TrendCharts({ company }) {
  ...
  const generateRevenueData = () => {
    ...
    const growth3yr = parseFloat(company.growth_metrics?.revenue_growth_3yr?.replace(/[^0-9.-]/g, '')) || 15;
    const growth5yr = parseFloat(company.growth_metrics?.revenue_growth_5yr?.replace(/[^0-9.-]/g, '')) || 12;
  ...
  const generateEmployeeData = () => {
    ...
    const employeeGrowth = parseFloat(company.growth_metrics?.employee_growth_rate?.replace(/[^0-9.-]/g, '')) || 10;
  ...
  const hasGrowthData = company.growth_metrics && 
    (company.growth_metrics.revenue_growth_3yr || company.growth_metrics.revenue_growth_5yr || company.growth_metrics.employee_growth_rate);
          ...
          {company.growth_metrics?.revenue_growth_5yr && (
            <div className="mt-3 text-center">
              <p className="text-sm text-gray-600">
                5-Year Growth: <span className="font-semibold text-green-600">{company.growth_metrics.revenue_growth_5yr}</span>
              </p>
            </div>
          )}
        </CardContent>
      ...
      <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
        ...
        <CardContent>
          ...
          {company.growth_metrics?.employee_growth_rate && (
            <div className="mt-3 text-center">
              <p className="text-sm text-gray-600">
                Growth Rate: <span className="font-semibold text-blue-600">{company.growth_metrics.employee_growth_rate}</span>
              </p>
            </div>
          )}
        </CardContent>
