...
export default function ABTestingPanel() {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.EmailABTest.create({ ...data, status: 'draft', winner: 'pending' }),
    onSuccess: () => {
        ...
        {tests.length === 0 ? (
          <Card className="glass-card rounded-2xl">
            <CardContent className="py-12 text-center">
              ...
              <p className="text-gray-500 mb-4">Test different variations to optimize performance</p>
              <Button onClick={() => setShowModal(true)} className="gap-2">
                <Plus className="w-4 h-4" />
                Create First Test
              </Button>
                ...
                <CardContent className="p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 capitalize">{test.test_type?.replace('_', ' ')}</p>
                    </div>
                    <Badge className={
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className={`p-4 rounded-lg border-2 ${stats.winner === 'a' && test.status === 'completed' ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20' : 'border-gray-200 dark:border-gray-700'}`}>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-300 mb-2 truncate">{test.variant_a?.value || 'Not set'}</p>
                      <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.aRate}%</p>
                      <p className="text-xs text-gray-500">{test.variant_a?.opens || 0} opens / {test.variant_a?.sent || 0} sent</p>
                    </div>

                    <div className={`p-4 rounded-lg border-2 ${stats.winner === 'b' && test.status === 'completed' ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20' : 'border-gray-200 dark:border-gray-700'}`}>
                      <div className="flex items-center justify-between mb-2">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-300 mb-2 truncate">{test.variant_b?.value || 'Not set'}</p>
                      <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.bRate}%</p>
                      <p className="text-xs text-gray-500">{test.variant_b?.opens || 0} opens / {test.variant_b?.sent || 0} sent</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <div className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, variant_a: { ...prev.variant_a, value: e.target.value } }))}
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, variant_b: { ...prev.variant_b, value: e.target.value } }))}
