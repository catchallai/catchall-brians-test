    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="text-center p-2 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg">
          <p className="text-xl font-bold text-emerald-600 dark:text-emerald-400">{latestData.top3 || 0}</p>
          <p className="text-xs text-gray-500 dark:text-gray-400">Top 3</p>
        </div>
        <div className="text-center p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          <p className="text-xl font-bold text-blue-600 dark:text-blue-400">{latestData.top10 || 0}</p>
        ...
        <div className="text-center p-2 bg-violet-50 dark:bg-violet-900/20 rounded-lg">
          <p className="text-xl font-bold text-violet-600 dark:text-violet-400">{latestData.top20 || 0}</p>
          <p className="text-xs text-gray-500 dark:text-gray-400">Top 20</p>
        </div>
        <div className="text-center p-2 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
          <p className="text-xl font-bold text-amber-600 dark:text-amber-400">{latestData.avgPosition || 0}</p>
      ...
      <div className="h-48">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey="top10" stroke="#3b82f6" strokeWidth={2} dot={false} name="Top 10" />
            <Line type="monotone" dataKey="top20" stroke="#8b5cf6" strokeWidth={2} dot={false} name="Top 20" />
          </LineChart>
        </ResponsiveContainer>
      </div>
        ...
        <div className="space-y-1">
          {topMovers.slice(0, 3).map((item, idx) => (
            <div key={idx} className="flex items-center justify-between text-sm py-1">
              <span className="text-gray-700 dark:text-gray-300 truncate flex-1">{item.keyword}</span>
