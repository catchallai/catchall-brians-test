...
import { Loader2, ShieldCheck, FileText, Image, Clock, Target, Eye, BarChart3, AlertCircle } from "lucide-react";
...
const TABS = [
  {
    ...
    items: [
      { key: 'copy_pool',     label: 'Copy Pool',          icon: FileText,   description: 'Approve written copy for all channels' },
      { key: 'templates',     label: 'Graphic Templates',  icon: Image,      description: 'Approve visual templates for all channels' },
      { key: 'brief',         label: 'Campaign Briefs',    icon: Target,     description: 'Build monthly multi-channel campaign briefs from approved assets' },
    ],
  },
  {
    group: 'Social Post Approvals',
    items: [
      { key: 'queue',   label: 'Approval Queue', icon: Clock,      description: 'Individual posts pending review' },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-violet-50 via-slate-50 to-cyan-50 dark:from-slate-950 dark:via-slate-900 dark:to-violet-950">
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-gray-900 dark:text-white">Content Approvals</h1>
                <p className="text-xs text-gray-500 dark:text-gray-400">Copy · Templates · Campaign Briefs · Social Posts</p>
              </div>
            </div>
            {totalPending > 0 && (
          ...
          <div className="flex gap-0 overflow-x-auto scrollbar-hide">
            {TABS.map((group, gi) => (
              <React.Fragment key={group.group}>
                {gi > 0 && <div className="w-px bg-gray-100 dark:bg-slate-700 self-stretch my-2 mx-2 flex-shrink-0" />}
                <div className="flex items-end">
                  {group.items.map(tab => {
                    const Icon = tab.icon;
                    const isActive = activeTab === tab.key;
                    const count = tab.key === 'queue' ? pendingPostCount
                      : tab.key === 'copy_pool' ? pendingCopy.length
                      : tab.key === 'templates' ? pendingTemplates.length
                      : 0;
                    return (
                      <button
                        key={tab.key}
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-6">
        {/* Copy Pool */}
        {activeTab === 'copy_pool' && (
          <CopyPoolTab currentUser={currentUser} />
        )}

        {/* Graphic Templates */}
        {activeTab === 'templates' && (
          <GraphicTemplatePoolTab currentUser={currentUser} />
        )}

        {/* Campaign Briefs */}
        {activeTab === 'brief' && (
          <CampaignBriefTab currentUser={currentUser} />
        )}
        ...
        {isLoading && ['queue','review','history','metrics'].includes(activeTab) ? (
          ...
          <>
            {activeTab === 'queue' && (
              <ApprovalQueueTab posts={allPosts} currentUser={currentUser} selectedPost={selectedPost} onSelectPost={setSelectedPost} statusLabels={STATUS_LABELS} />
            )}
            {activeTab === 'review' && (
              <ApprovalReviewTab posts={allPosts} currentUser={currentUser} selectedPost={selectedPost} onSelectPost={setSelectedPost} statusLabels={STATUS_LABELS} />
            )}
            {activeTab === 'history' && (
              <ApprovalHistoryTab posts={allPosts} currentUser={currentUser} statusLabels={STATUS_LABELS} />
            )}
            {activeTab === 'metrics' && (
              <ApprovalMetricsTab posts={allPosts} currentUser={currentUser} statusLabels={STATUS_LABELS} />
            )}
          </>
        )}
      </div>
