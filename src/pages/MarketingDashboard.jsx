...
export default function MarketingDashboard() {
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 min-h-screen">
      <div>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Unified Marketing Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="glass-card">
          <CardContent className="pt-6">
            ...
            <p className="text-3xl font-bold text-violet-600">{latestMetrics.organic_traffic?.toLocaleString() || 0}</p>
            <p className="text-xs text-gray-400 mt-1">ROI: {latestMetrics.organic_roi?.toFixed(0) || 0}%</p>
          </CardContent>
        </Card>
        <Card className="glass-card">
          <CardContent className="pt-6">
            <p className="text-sm text-gray-500">Social Traffic</p>
            <p className="text-3xl font-bold text-emerald-600">{latestMetrics.social_traffic?.toLocaleString() || 0}</p>
            <p className="text-xs text-gray-400 mt-1">ROI: {latestMetrics.social_roi?.toFixed(0) || 0}%</p>
          </CardContent>
        </Card>
        <Card className="glass-card">
          <CardContent className="pt-6">
            <p className="text-sm text-gray-500">Organic Revenue</p>
            <p className="text-3xl font-bold text-violet-600">${(totalOrganic / 1000).toFixed(1)}K</p>
          </CardContent>
        </Card>
        <Card className="glass-card">
          <CardContent className="pt-6">
            <p className="text-sm text-gray-500">Social Revenue</p>
            <p className="text-3xl font-bold text-emerald-600">${(totalSocial / 1000).toFixed(1)}K</p>
          </CardContent>
        </Card>
      </div>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={metrics.slice(-30)}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Line type="monotone" dataKey="organic_revenue" stroke="#8b5cf6" name="Organic" />
          ...
          <Card className="glass-card">
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart
                  data={[
                    { name: 'Organic', traffic: latestMetrics.organic_traffic || 0, conversions: latestMetrics.organic_conversions || 0 },
                    { name: 'Social', traffic: latestMetrics.social_traffic || 0, conversions: latestMetrics.social_conversions || 0 }
                  ]}
                >
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="name" />
          ...
          <Card className="glass-card">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-4">Organic (SEO)</h3>
                  <div className="space-y-3">
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">Traffic</p>
                      <p className="text-2xl font-bold">{latestMetrics.organic_traffic?.toLocaleString() || 0}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">Conversions</p>
                      <p className="text-2xl font-bold">{latestMetrics.organic_conversions?.toLocaleString() || 0}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">Conversion Rate</p>
                      <p className="text-2xl font-bold text-violet-600">
                        {latestMetrics.organic_traffic 
                          ? ((latestMetrics.organic_conversions || 0) / latestMetrics.organic_traffic * 100).toFixed(2)
                          : 0}%
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">ROI</p>
                      <p className="text-2xl font-bold text-violet-600">{latestMetrics.organic_roi?.toFixed(0) || 0}%</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">Traffic</p>
                      <p className="text-2xl font-bold">{latestMetrics.social_traffic?.toLocaleString() || 0}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">Conversions</p>
                      <p className="text-2xl font-bold">{latestMetrics.social_conversions?.toLocaleString() || 0}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">Conversion Rate</p>
                      <p className="text-2xl font-bold text-emerald-600">
                        {latestMetrics.social_traffic 
                          ? ((latestMetrics.social_conversions || 0) / latestMetrics.social_traffic * 100).toFixed(2)
                          : 0}%
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">ROI</p>
                      <p className="text-2xl font-bold text-emerald-600">{latestMetrics.social_roi?.toFixed(0) || 0}%</p>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <tbody>
                    {topKeywords.map((kw) => (
                      <tr key={kw.id} className="border-b border-gray-100 dark:border-gray-800">
                        ...
                        <td className="text-right py-2 px-3 font-semibold">#{kw.current_position || 'N/A'}</td>
                        <td className="text-right py-2 px-3">{kw.search_volume?.toLocaleString() || 0}</td>
                        <td className="text-right py-2 px-3">
                          {kw.previous_position && kw.current_position < kw.previous_position 
                            ? <span className="text-emerald-600">↑ {kw.previous_position - kw.current_position}</span>
              ...
              {topPosts.map((post) => (
                <div key={post.id} className="p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                  <div className="flex justify-between items-start mb-2">
                    <p className="font-medium text-gray-900 dark:text-white text-sm">{post.platform}</p>
                    <p className="text-xs text-gray-500">{new Date(post.created_date).toLocaleDateString()}</p>
                  </div>
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-2 line-clamp-2">{post.content}</p>
