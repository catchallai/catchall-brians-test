...
import {
  Plus, FileText, Code, Eye, Trash2, Settings, Inbox, 
...
export default function ContactForms() {
  ...
  const getNewSubmissionCount = (formId) => submissions.filter(s => s.form_id === formId && s.status === 'new').length;
    ...
    <div className="p-6 lg:p-8 space-y-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">Create and manage contact forms for your websites</p>
        </div>
        <Button onClick={() => { setEditingForm(null); setShowBuilder(true); }} className="gap-2 bg-violet-600 hover:bg-violet-700">
          ...
          {forms.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {forms.map((form) => {
                ...
                return (
                  <Card key={form.id} className="border-0 shadow-sm bg-white dark:bg-gray-800 hover:shadow-md transition-all">
                    <CardContent className="p-5">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <Badge className={form.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'}>
                      ...
                      <div className="flex items-center gap-4 mb-4 text-sm text-gray-500 dark:text-gray-400">
                        ...
                        {newCount > 0 && (
                          <Badge className="bg-amber-100 text-amber-700 text-xs">{newCount} new</Badge>
                        )}
                      </div>
