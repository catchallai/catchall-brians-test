...
export default function SocialPerformance() {
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 min-h-screen">
      <div>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Social Media Performance</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="glass-card">
          <CardContent className="pt-6">
            ...
            <p className="text-3xl font-bold text-violet-600">{(totalFollowers/1000).toFixed(1)}K</p>
          </CardContent>
        </Card>
        <Card className="glass-card">
          <CardContent className="pt-6">
            <p className="text-sm text-gray-500">Social Conversions</p>
            <p className="text-3xl font-bold text-emerald-600">{totalConversions.toLocaleString()}</p>
          </CardContent>
        </Card>
        <Card className="glass-card">
          <CardContent className="pt-6">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={metrics.slice(-30)}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Line yAxisId="left" type="monotone" dataKey="social_conversions" stroke="#8b5cf6" name="Conversions" />
                    <Line yAxisId="right" type="monotone" dataKey="social_roi" stroke="#10b981" name="ROI %" />
                  </LineChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-sm">{metrics[metrics.length-1]?.organic_traffic || 0}</span>
                  </div>
                ...
                <div>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-sm">{metrics[metrics.length-1]?.social_traffic || 0}</span>
                  </div>
            ...
            {accounts.slice(0, 3).map((account) => (
              ...
              >
                <CardContent className="pt-6">
                  ...
                  <p className="text-2xl font-bold mt-2">{(account.followers_count/1000).toFixed(1)}K</p>
                </CardContent>
              </Card>
            ))}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card className="glass-card">
                ...
                <CardContent className="space-y-3">
                  <div>
                    <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Top Countries</p>
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Growth Rate</p>
                    <p className="text-lg font-bold text-emerald-600">+{audience.follower_growth_rate}%/mo</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="glass-card">
                ...
                <CardContent className="space-y-3">
                  <div>
                    <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Top Interests</p>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {audience.interests?.slice(0, 4).map((interest) => (
                        <Badge key={interest} className="bg-violet-100 text-violet-800">{interest}</Badge>
                      ))}
                    </div>
                  </div>
                </CardContent>
            ...
            <Card className="glass-card">
              ...
              <CardContent className="space-y-3">
                {mentions.slice(0, 8).map((mention) => (
                  <div key={mention.id} className="p-3 rounded-lg border border-gray-200 dark:border-gray-700">
                    <div className="flex items-start justify-between mb-2">
                      <p className="text-sm font-medium text-gray-900 dark:text-white">{mention.author}</p>
                      <Badge className={`${
                        mention.sentiment === 'positive' ? 'bg-emerald-100 text-emerald-800' :
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">{mention.mention_text}</p>
                    <p className="text-xs text-gray-500">{mention.source} • {mention.reach?.toLocaleString()} reach</p>
                  </div>
                ))}
              </CardContent>
            </Card>
