...
import {
  ...
  DollarSign, Activity, Clock, Phone, UserPlus, Award, BarChart3, AlertCircle
} from "lucide-react";
...
export default function CRMDashboard() {
  ...
  const stats = {
    ...
    deals: {
      ...
      totalValue: deals.filter(d => !['won', 'lost'].includes(d.stage)).reduce((sum, d) => sum + (d.value || 0), 0),
    ...
    emails: {
      ...
      openRate: emailLogs.length > 0 ? Math.round((emailLogs.filter(e => e.status === 'opened' || e.status === 'clicked').length / emailLogs.length) * 100) : 0,
    },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-green-600 dark:text-green-400 mt-0.5">+{stats.contacts.newThisWeek} this week</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-white">${(stats.deals.totalValue / 1000).toFixed(0)}K</p>
                <p className="text-xs text-gray-500 mt-1">{stats.deals.active} active deals</p>
                <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5">Avg: ${(stats.deals.avgDealSize / 1000).toFixed(0)}K</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-white">${(stats.deals.wonValue / 1000).toFixed(0)}K</p>
                <p className="text-xs text-gray-500 mt-1">{stats.deals.won} deals closed</p>
                <p className="text-xs text-violet-600 dark:text-violet-400 mt-0.5">{stats.opportunities.won} opportunities won</p>
              </div>
        ...
        <Card className="glass-card">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Email Performance</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.emails.openRate}%</p>
                <p className="text-xs text-gray-500 mt-1">{stats.emails.sent} emails sent</p>
                <p className="text-xs text-cyan-600 dark:text-cyan-400 mt-0.5">{stats.emails.clicked} clicked</p>
              </div>
          ...
          <CardContent>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-semibold text-gray-900 dark:text-white">{stats.activities.thisWeek}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-gray-400">Notes</span>
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{stats.activities.byType.notes}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-gray-400">Calls</span>
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{stats.activities.byType.calls}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-gray-400">Meetings</span>
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{stats.activities.byType.meetings}</span>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-semibold text-gray-900 dark:text-white">{stats.companies.total}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-gray-400">New This Month</span>
                <span className="text-sm font-medium text-green-600 dark:text-green-400">+{stats.companies.newThisMonth}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-gray-400">Open Opportunities</span>
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{stats.opportunities.open}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-gray-400">Qualified</span>
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{stats.opportunities.qualified}</span>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-semibold text-gray-900 dark:text-white">{stats.contacts.leads}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-gray-400">Prospects</span>
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{stats.contacts.prospects}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-gray-400">Customers</span>
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{stats.contacts.customers}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600 dark:text-gray-400">New This Month</span>
                <span className="text-sm font-medium text-green-600 dark:text-green-400">+{stats.contacts.newThisMonth}</span>
              </div>
            </div>
          </CardContent>
        </Card>
